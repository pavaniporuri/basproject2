...
const assertError = (code, element, value, key, pathSegments = []) => {
  ...
  const error = getEntry({ code, message: code, target: path, args: [name || key] })
  Object.assign(error, {
...
const checkInputConstraints = ({ element, value, errors, key, pathSegments }) => {
  ...
  if (enumElements && !_enumValues(enumElements).includes(value)) {
    errors.push(assertError(ASSERT_ENUM, element, value, key, pathSegments))
  }
  ...
  if (rangeElements && !_checkInRange(value, rangeElements)) {
    errors.push(assertError(ASSERT_RANGE, element, value, key, pathSegments))
  }

  // format
  const formatElements = element['@assert.format']
  if (formatElements && !_checkRegExpFormat(value, formatElements)) {
    errors.push(assertError(ASSERT_FORMAT, element, value, key, pathSegments))
  }
