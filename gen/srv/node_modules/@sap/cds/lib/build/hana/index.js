...
class HanaModuleBuilder extends BuildTaskHandlerOData {
    ...
    async _compileToHdbmigration(model, pluginTypes, options) {
        ...
        let definitions, afterImage = null

        try {
            const compilationResult = await this.cds.compile.to.hdbmigration(model, lastDev, dbSrcDir, options)
            definitions = compilationResult.definitions
            afterImage = compilationResult.afterImage
        } finally {
            // ensure compiler messages are added in case this.cds.compile.to.hdbmigrationtable is throwing an error
            this.pushMessages(options.messages)
        }
        const promises = []
            ...
            if (suffix === FILE_EXT_HDBMIGRATIONTABLE) {
                ...
                if (suffix === FILE_EXT_HDBTABLE) {
                    ...
                    if (await fs.pathExists(path.join(dbSrcDir, name + FILE_EXT_HDBMIGRATIONTABLE))) {
                        this.pushMessages(new BuildError(`Multiple files exist defining the same HANA artifact - [${path.join(relDbSrcDir, name + FILE_EXT_HDBMIGRATIONTABLE)}, ${path.join(relDbDestDir, file)}].\nEither annotate the model entity using @cds.persistence.journal or undeploy the file [${path.join('src', name + FILE_EXT_HDBMIGRATIONTABLE)}] using an undeploy.json file.`))
                    }
                }
            }
        }
        ...
        if (afterImage) {
            if (migrationTableFiles.length > 0) {
                this._result.hana.push(lastDevCsnFolder)
...
applications:
  - name: ${descriptor.appName}
    ...
    async _validateHdbmigrationtables() {
        const dbSrcDir = path.join(this.task.src, "src")
        const migrationTableFiles = BuildTaskHandlerOData._find(dbSrcDir, (res) => {
            return fs.statSync(res).isFile() && path.extname(res) === FILE_EXT_HDBMIGRATIONTABLE
        })
        if (migrationTableFiles.length > 0) {
            const { MigrationTableParser } = require('../../utils/migrationtable')
            ...
            await Promise.all(migrationTableFiles.map(async file => {
                try {
                    ...
                    if (tableModel && /^>>>>>/m.test(tableModel.migrations.toString())) {
                        messages.push(new BuildError(`Current model changes require manual resolution. See migration file ${path.relative(this.buildOptions.root, file)} for further details.`))
                    }
                } catch (e) {
                    // do not abort build in post validation step
                    messages.push(new BuildError(`${path.relative(this.buildOptions.root, file)}: ${e.toString()}`))
                }
            }))
