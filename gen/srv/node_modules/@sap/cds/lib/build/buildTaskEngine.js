...
class BuildTaskEngine {
    ...
    async _doPrepare(handlerGroup) {
        for (let handler of handlerGroup) {
            if (DEBUG) {
                this.logger.log(`[cds] - preparing, handler [${handler.name}], src [${relativePaths(handler.buildOptions.root, handler.task.src)}]`)
            }
    ...
    async _executeCleanBuildTasks(handlers, buildOptions, clean) {
        if (clean) {
            ...
            const results = await allSettled(handlers.map((handler) => {
                if (DEBUG) {
                    this.logger.log(`[cds] - cleaning, handler [${handler.name}], src [${relativePaths(handler.buildOptions.root, handler.task.src)}]`)
                }
    ...
    async _executePipeline(buildOptions, pipeline) {
        ...
        for (const group of pipeline) {
            const results = await allSettled(group.map((handler) => {
                if (DEBUG) {
                    this.logger.log(`[cds] - building, handler [${handler.name}], src [${relativePaths(handler.buildOptions.root, handler.task.src)}]`)
                }
    ...
    _sortCompilationErrorsUnique(buildOptions, ...messages) {
        ...
        const filteredMessages = messages.filter(message => !message.severity || logLevelIdx >= SEVERITES.indexOf(message.severity))
        // remove duplicates
