...
const { ensureNoDraftsSuffix, ensureDraftsSuffix, draftIsLocked } = require('./draftUtils')
...
const validateDraft = (draftResult, req) => {
...
const getDeepSelect = req => {
  ...
  for (const each in elements) {
    ...
    // REVISIT: remove feature flag skip_expand_assoc after grace period of at least two months (> April release)
    else if (e._isAssociationEffective && cds.env.features.skip_expand_assoc === false) {
      const skip = e._target['@cds.persistence.skip']
      if (skip && skip !== 'if-unused') continue
      if (e.is2one && e.keys) {
        const keys = foreignKeyPropagations(e, true, false)
        if (keys) {
          const refs = keys.map(k => ({ ref: [k.childFieldName] }))
          cols.push({ ref: [each], expand: refs })
        }
      }
